...
function createElements(response) {
  ...
  document.getElementById("mars-date").textContent =
    "SOL " + response.sol_keys[length];
  ...
  if ("AT" in response[sol]) {
    HIGH_C =
      response[sol].AT.mx === null ? "N/A" : response[sol].AT.mx.toFixed(3);
    HIGH_F =
      response[sol].AT.mx === null
        ? "N/A"
        : (response[sol].AT.mx * (9 / 5) + 32).toFixed(3);
  }
  ...
  if ("AT" in response[sol]) {
    LOW_C =
      response[sol].AT.mn === null ? "N/A" : response[sol].AT.mn.toFixed(3);
    LOW_F =
      response[sol].AT.mn === null
        ? "N/A"
        : (response[sol].AT.mn * (9 / 5) + 32).toFixed(3);
  }

  let WD = "N/A";
  if ("most_common" in response[sol]) {
    WD =
      response[sol].WD.most_common === null
        ? "N/A"
        : response[sol].WD.most_common.compass_point;
  }
  ...
  if ("PRE" in response[sol]) {
    AP =
      response[sol].PRE.mx === null ? "N/A" : response[sol].PRE.mx.toFixed(5);
  }
  ...
  document.getElementById("wind-speed").textContent =
    "Wind Speed: " + WS + " (m/s)";
  document.getElementById("atmospheric-pressure").textContent =
...
function buildTable(data) {
  ...
  let highTempRow_c =
    "<tr id='tr-high-c' class='sol-table-body-row' style='display: inline-list-item;'/>";
  let lowTempRow_c =
    "<tr id='tr-low-c' class='sol-table-body-row' style='display: inline-list-item;'/>";
  let highTempRow_f =
    "<tr id='tr-high-f' class='sol-table-body-row' style='display: none;'/>";
  let lowTempRow_f =
    "<tr id='tr-low-f' class='sol-table-body-row' style='display: none;'/>";
  ...
  for (let i = 0; i < solKeys.length; i++) {
    ...
    solRow += "<td>" + sol + "</td>";
    earthDateRow +=
      "<td style='border-bottom-style: solid'>" + newDate + "</td>";
    ...
    highTempRow_c +=
      "<td id='td-high-c'>High: " +
      high_temp_c +
      "<span>&deg;&nbsp;C</span></td>";
    lowTempRow_c +=
      "<td id='td-low-c'>Low: " + low_temp_c + "<span>&deg;&nbsp;C</span></td>";
    highTempRow_f +=
      "<td id='td-high-f'>High: " +
      high_temp_f +
      "<span>&deg;&nbsp;F</span></td>";
    lowTempRow_f +=
      "<td id='td-low-f'>Low: " + low_temp_f + "<span>&deg;&nbsp;F</span></td>";
  }
...
function toggleTemp(c_f) {
  if (c_f == "F") {
    ...
    document.getElementById("lbl-temp-high-f").style =
      "color: grey; cursor: text;";
